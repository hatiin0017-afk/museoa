...
function initWorkflows(){
  ...
  function renderUpbo(){
    ...
    rows.forEach(r=>{
      ...
      actions.append(plus,minus,done,ready);card.append(info,actions);list.append(card);
    });
  ...
  $('#season-form').addEventListener('submit',async event=>{event.preventDefault();const name=event.target.elements.name.value.trim();if(!name)return;if(await A.commit(d=>{if(!d.seasons.includes(name))d.seasons.push(name);})){event.target.reset();$('#assign-season').value=name;members();renderUpbo();}});
  ...
  function render(){
    ...
    const seasons=[...new Set(['시즌 1',...data.seasons,...data.upbo.map(r=>r.season)])];for(const id of ['assign-season','stats-season']){const select=$('#'+id),previous=select.value;select.replaceChildren();if(id==='stats-season')select.append(new Option('전체 시즌',''));seasons.forEach(s=>select.append(new Option(s,s)));if([...select.options].some(o=>o.value===previous))select.value=previous;}$('#season-list').replaceChildren();seasons.forEach(s=>$('#season-list').append(node('p',s)));members();
