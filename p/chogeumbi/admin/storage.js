...
function initStorage(){
  ...
  async function clean(image){
    ...
    try{
      ...
      if(latest.payload.outfits.some(o=>o.image===image))throw new Error('다른 의상이 같은 이미지를 사용하고 있어 보존했습니다.');
  ...
  window.GEUMBI_STORAGE={path,clean,upload};addEventListener('geumbi-admin-render',pending);pending();
  ...
  async function upload(){
    ...
    try{
      ...
      form.elements.image.value=data.publicUrl;form.elements.image.dispatchEvent(new Event('input'));result.textContent='이미지 업로드 완료';return true;
