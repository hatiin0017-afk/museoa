...
(async () => {
  ...
  async function save(next){
    ...
    CHOGEUMBI_MODEL.normalize(next);await GEUMBI.save(next);data=next;snapshot=localStorage.getItem(GEUMBI.key);render();feedback(GEUMBI.local?'로컬 예시에 저장했습니다.':'Supabase에 저장했습니다. 공개 페이지에도 반영됩니다.');
  }
  let saving=false;
  ...
  $('#settings-form').addEventListener('submit',e=>{e.preventDefault();const values=entries(e.target);values.restDay=Number(values.restDay);const birthday=/^(\d{2})\.(\d{2})$/.exec(values.birthday),debut=/^(\d{4})\.(\d{2})\.(\d{2})$/.exec(values.debut);function valid(y,m,d){const dt=new Date(Date.UTC(y,m-1,d));return dt.getUTCFullYear()===y&&dt.getUTCMonth()===m-1&&dt.getUTCDate()===d;}if(!birthday||!valid(2000,+birthday[1],+birthday[2])||!debut||!valid(+debut[1],+debut[2],+debut[3])){feedback('생일과 데뷔일을 실제 날짜 형식으로 입력해 주세요.',true);return;}commit(next=>next.settings=values);});
  let uploaded='';
